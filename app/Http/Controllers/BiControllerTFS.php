...
class BiControllerTFS extends Controller
{
    public function BiPanel()
    {
        ...
        if ($webStatus->web_status === 1) {
            ...
            if (Auth::user() == null) {
                ...
                return view('bi_client_tfs.bi-client-master', compact('javs','tors'));
            }
...
//                        return '<a class="btn btn-xs btn-success btn-block" data-toggle="modal" data-target="" disabled><i class="fa fa-fw fa-check-square"></i> Finished</a>'.
                    ...
                    else if($get_general_table->status == 2)
                    {
                        ...
                        $now = Carbon::now('Asia/Manila');
                        $answer = $now->diffInMinutes($date, false);
...
//                        }
                    ...
                    else if($get_general_table->status == 3)
                    {
                        ...
                        if($answer > 0)
                        {
                            ...
                            {
                                return $succveri.'<a class="btn btn-xs btn-success btn-block" data-toggle="modal" data-target="" disabled><i class="glyphicon glyphicon-time"></i> '.$difference_days.' Days </a>';
                            }
                            else if($difference_hours <= -1)
                            {
                                return $succveri.'<a class="btn btn-xs bg-black btn-block" data-toggle="modal" data-target="" disabled><i class="glyphicon glyphicon-time"></i> LATE </a>';
                            ...
                            {
                                return $succveri.'<a class="btn btn-xs btn-warning btn-block" data-toggle="modal" data-target="" disabled><i class="glyphicon glyphicon-time"></i> '.$difference_hours.' Hours </a>';
                            }
                            else if($difference_mins <= -1)
                            {
            ...
            ->editColumn('button_edit', function($data)
            {
                $addCancel = '';
                $editReq = '';
                ...
                if($data->cancel_status == '' || $data->cancel_status == null)
                {
                    ...
                    {
                        $editReq = '<button class="btn btn-warning btn-xs btn-block client_edit_req" id="'.$data->endorse_id.'"><span class="glyphicon glyphicon-pencil"></span>Edit Endorsement Details</button>';
                ...
                return $editReq . $addCancel . '<a id="'.$data->endorse_id.'" class="btn_view_information_bi btn btn-xs btn-info btn-block" data-toggle="modal" data-target="" ><i class="glyphicon glyphicon-film"></i> View Information</a>';
            })
            ->rawColumns([
                'attachments',
                'due',
        ...
        return DataTables::of($get_general_table)
            ...
            ->editColumn('due', function($get_general_table)
            {
                ...
                else
                {
                    if($get_general_table->due == 2)
                    {
                        return $ackhtml .'<a class="btn btn-xs bg-black btn-block" data-toggle="modal" data-target="" disabled><i class="glyphicon glyphicon-time"></i> LATE </a>';
                    }
                    else if($get_general_table->due)
                    {
                        return $pendhtml .'<a class="btn btn-xs bg-black btn-block" data-toggle="modal" data-target="" disabled><i class="glyphicon glyphicon-time"></i> LATE </a>';
                    }
                }
            })
            ->editColumn('check', function ($query)
...
//                ->groupBy('bi_endorsements_checkings.bi_endorsement_id')
                ...
                ->where('bi_endorsements.cancel_bool', '!=', 'Cancelled')
                ->where('bi_endorsements.type_of_endorsement_bank', '=', null);
        }
...
//            ->leftjoin('bi_endorsements_checkings','bi_endorsements_checkings.bi_endorsement_id','=','bi_endorsements.id')
            ...
            ([
                'bi_endorsements.contract_num as contract_num',
                'bi_endorsements.dealer_num as dealer_num',
                'bi_endorsements.id as endorse_id',
                'bi_endorsements.bi_account_name as site',
                'bi_endorsements.created_at as date_time_endorsed',
                'bi_endorsements.project as project',
...
//        return response()->json($other_address);
    ...
    public function bi_submit_endorsement_files(Request $request)
    {
        ...
        if($file1 != null)
        {
            ...
            $file1->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$ongoing_id.'/'),preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name1));

            $endorse->attach_1 = preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name1);
        ...
        if($file2 != null)
        {
            ...
            $file2->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$ongoing_id.'/'),preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name2));

            $endorse->attach_2 = preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name2);
        ...
        if($file3 != null)
        {
            ...
            $file3->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$ongoing_id.'/'),preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name3));

            $endorse->attach_3 = preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name3);
        ...
        if($file4 != null)
        {
            ...
            $file4->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$ongoing_id.'/'),preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name4));

            $endorse->attach_4 = preg_replace('/[^a-zA-Z0-9\-\._]/','', $file_name4);
...
//             ->count();
        ...
        return response()->json([0, 0 , 0 , 0, 0]);
    }
            ...
            for($v = 0; $v < count($arrayToInsert); $v++)
            {
                ...
                $getId = DB::table('bi_endorsements')
                    ...
                    ([
                        ...
                        'present_muni' => $presentMuni[0]->id,
                        'present_province' => $presentProv[0]->id,
                        'permanent_address' =>  $arrayToInsert[$v][17],
                        'permanent_muni' => $permaMuni[0]->id,
                        'permanent_province' => $permaProv[0]->id,
...
//                    'bi_endorsements_checkings.checking_name as check',
    ...
    public function bi_client_hold_table()
    {
...
//        $maidenname = $removeScript->scripttrim($request->acct_maiden_last_name) . ', ' . $removeScript->scripttrim($request->acct_maiden_first_name) . ' ' . $removeScript->scripttrim($request->acct_maiden_middle_name);
        ...
        $endorseId = DB::table('bi_endorsements')
            ...
            ([
                ...
                'dealer_num' => $removeScript->scripttrim($request->dealer_num),
...
//        }
        ...
        $email = new EmailQueries();
        $email->SendEndorsementNotifToSAO(Auth::user()->id, $removeScript->scripttrim($request->acct_last) . ', ' . $removeScript->scripttrim($request->acct_first) . ' ' . $removeScript->scripttrim($request->acct_middle));
        ...
        return DataTables::of($get_pending_applicants)
            ->editColumn('attachments', function ($query)
            {
                ...
                else
                {
                    ...
                    {
                        $downloads .= '1. '.$get_attachment[0]->attach1.'<br><a class="btn_download_bi_files btn btn-xs btn-info btn-block" id="'.$query->id.'" name="1" href="get_encoded_file/'.base64_encode($query->id).'/'.base64_encode($get_attachment[0]->attach1).'"><i class="glyphicon glyphicon-download-alt"></i> Download</a>';
                    }
                    else
                    {
                        $downloads .='<p>1. none</p>';
                    ...
                    {
                        $downloads .= '2. '.$get_attachment[0]->attach2.'<br><a class="btn_download_bi_files btn btn-xs btn-info btn-block" id="'.$query->id.'" name="2" href="get_encoded_file/'.base64_encode($query->id).'/'.base64_encode($get_attachment[0]->attach2).'"><i class="glyphicon glyphicon-download-alt"></i> Download</a>';
                    }
                    else
                    {
                        $downloads .='<p>2. none</p>';
                    ...
                    {
                        $downloads .= '3. '.$get_attachment[0]->attach3.'<br><a class="btn_download_bi_files btn btn-xs btn-info btn-block" id="'.$query->id.'" name="3" href="get_encoded_file/'.base64_encode($query->id).'/'.base64_encode($get_attachment[0]->attach3).'"><i class="glyphicon glyphicon-download-alt"></i> Download</a>';
                    }
                    else
                    {
                        $downloads .='<p>3. none</p>';
                    ...
                    {
                        $downloads .= '4. '.$get_attachment[0]->attach4.'<br><a class="btn_download_bi_files btn btn-xs btn-info btn-block" id="'.$query->id.'" name="4" href="get_encoded_file/'.base64_encode($query->id).'/'.base64_encode($get_attachment[0]->attach4).'"><i class="glyphicon glyphicon-download-alt"></i> Download</a>';
                    }
                    else
                    {
                        $downloads .='<p>4. none</p>';
...
//            ->toJson();
    ...
    public function bi_endorse_encoded_account(Request $request)
    {
        ...
        if($getInfo[0]->attach1 != '')
        {
            if(file_exists(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id. '/'. $request->id.'/'. $getInfo[0]->attach1)))
            {
                File::move(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id.'/'. $request->id.'/'.$getInfo[0]->attach1) , storage_path('bi_attachments/'.$getBIid[0]->bi_id. '/'. $endorse_id.'/'.$getInfo[0]->attach1));
            }
        }

        if($getInfo[0]->attach2 != '')
        {
            if(file_exists(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id. '/'. $request->id.'/'. $getInfo[0]->attach2)))
            {
                File::move(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id.'/'. $request->id.'/'.$getInfo[0]->attach2) , storage_path('bi_attachments/'.$getBIid[0]->bi_id. '/'. $endorse_id.'/'.$getInfo[0]->attach2));
            }
        }


        if($getInfo[0]->attach3 != '')
        {
            if(file_exists(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id. '/'. $request->id.'/'. $getInfo[0]->attach3)))
            {
                File::move(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id.'/'. $request->id.'/'.$getInfo[0]->attach3) , storage_path('bi_attachments/'.$getBIid[0]->bi_id. '/'. $endorse_id.'/'.$getInfo[0]->attach3));
            }
        }


        if($getInfo[0]->attach4 != '')
        {
            if(file_exists(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id. '/'. $request->id.'/'. $getInfo[0]->attach4)))
            {
                File::move(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id.'/'. $request->id.'/'.$getInfo[0]->attach4) , storage_path('bi_attachments/'.$getBIid[0]->bi_id. '/'. $endorse_id.'/'.$getInfo[0]->attach4));
            }
        }

        File::deleteDirectory(storage_path('bi_attachments_direct/'.$getBIid[0]->bi_id.'/'.$request->id));
    ...
    public function tfs_bi_client_check_endorse_time(Request $request)
    {
        $getTime = DB::table('bi_endorsements')
            ->where('id', $request->id)
            ->select([
                'contract_num',
                'dealer_num',
                ...
                'm_name',
                'client_remarks_bank',
                'type_of_endorsement_bank as tor',
                'created_at as endorse_date'
            ])
            ->get();

       if(Carbon::now('Asia/Manila')->diffInMinutes($getTime[0]->endorse_date) <= 5)
       {
           return response()->json(['go', $getTime]);
       }
       else
       {
           return 'exceeded';
...
//         return response()->json(['go', $getTime]);
    ...
    public function tfs_bi_upload_edited_endo_file(Request $request)
    {
        $data_index = explode('||-||-||', $request->data_index);
        $data = json_decode($request->data);
        $what = '';

        $endorse = new bi_endorsement();
        $endorse = $endorse::find($request->id);
        $like_ctr = 1;
        $get_names = [];
        $what_to_update = '';
        $file_index = [];
        $logs = '';
        $attach_1 = $request->file('attach_1');
        $attach_2 = $request->file('attach_2');
        $attach_3 = $request->file('attach_3');
        $attach_4 = $request->file('attach_4');
        $file_index = explode("||-||-||", $request->file_names);
        $for_logs = '';

        if($attach_1 != null)
        {
            $file_name1 = $attach_1->getClientOriginalName();

            if($file_name1 == $endorse->attach_2)
            {
                $like_ctr++;
                $file_name1 = explode('.',$attach_1->getClientOriginalName())[0].'('.$like_ctr.').'.$attach_1->getClientOriginalExtension();
            }
            ...
            if($endorse->attach_1 != $file_name1)
            {
                $logs = $endorse->attach_1;
                File::delete(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'. $endorse->attach_1));

                $attach_1->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'),$file_name1);

                $endorse->attach_1 = $file_name1;
                $for_logs .= strtoupper('ATTACHMENT 1: FILE ' . $logs . ' WAS UPDATED TO ' . $file_name1 . ', ');
            }
            else
            {
                $for_logs .= strtoupper('ATTACHMENT 1: FILE ' . $file_name1 . ' WAS UPDATED, ');
            }
        }
        else
        {
        ...
        if($attach_2 != null)
        {
            $file_name2 = $attach_2->getClientOriginalName();

            if($file_name2 == $endorse->attach_1)
            {
                $like_ctr++;
                $file_name2 = explode('.',$attach_2->getClientOriginalName())[0].'('.$like_ctr.').'.$attach_2->getClientOriginalExtension();
            }
            ...
            if($endorse->attach_2 != $file_name2)
            {
                $logs = $endorse->attach_2;
                File::delete(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'.$endorse->attach_2));

                $attach_2->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'),$file_name2);

                $endorse->attach_2 = $file_name2;
                $for_logs .= strtoupper('ATTACHMENT 2: FILE ' . $logs . ' WAS UPDATED TO ' . $file_name2 . ', ');
            }
            else
            {
                $for_logs .= strtoupper('ATTACHMENT 2: FILE ' . $file_name2 . ' WAS UPDATED, ');
            }
        }
        else
        {
        ...
        if($attach_3 != null)
        {
            $file_name3 = $attach_3->getClientOriginalName();

            if($file_name3 == $endorse->attach_1)
            {
                $like_ctr++;
                $file_name3 = explode('.',$attach_3->getClientOriginalName())[0].'('.$like_ctr.').'.$attach_3->getClientOriginalExtension();
            }
            ...
            if($endorse->attach_3 != $file_name3)
            {
                $logs = $endorse->attach_3;
                File::delete(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'.$endorse->attach_3));

                $attach_3->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'),$file_name3);

                $endorse->attach_3 = $file_name3;
                $for_logs .= strtoupper('ATTACHMENT 3: FILE ' . $logs . ' WAS UPDATED TO ' . $file_name3 . ', ');
            }
            else
            {
                $for_logs .= strtoupper('ATTACHMENT 3: FILE ' . $file_name3 . ' WAS UPDATED, ');
            }
        }
        else
        {
        ...
        if($attach_4 != null)
        {
            $file_name4 = $attach_4->getClientOriginalName();

            if($file_name4 == $endorse->attach_1)
            {
                $like_ctr++;
                $file_name4 = explode('.',$attach_4->getClientOriginalName())[0].'('.$like_ctr.').'.$attach_4->getClientOriginalExtension();
            }
            ...
            if($file_name4 == $endorse->attach_3)
            {
                $like_ctr++;
                $file_name4 = explode('.',$attach_4->getClientOriginalName())[0].'('.$like_ctr.').'.$attach_4->getClientOriginalExtension();
            }

            if($endorse->attach_4 != $file_name4)
            {
                $logs = $endorse->attach_4;
                File::delete(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'.$endorse->attach_4));

                $attach_4->move(storage_path('bi_attachments/'.$endorse->bi_id.'/'.$request->id.'/'),$file_name4);

                $endorse->attach_4 = $file_name4;
                $for_logs .= strtoupper('ATTACHMENT 4: FILE ' . $logs . ' WAS UPDATED TO ' . $file_name4 . ', ');
            }
            else
            {
                $for_logs .= strtoupper('ATTACHMENT 4: FILE ' . $file_name4 . ' WAS UPDATED, ');
            }
        }
        else
        {
        ...
        $for_logs = substr($for_logs, 0, -2);

        for($ii = 0; $ii < count($data_index); $ii++)
        {
            $what = $data_index[$ii];
            $endorse->$what = $data->$what;
        }
        $endorse->account_name = $data->l_name . ', ' . $data->f_name . ' ' . $data->m_name;
        $endorse->save();

        $logs = new bi_log();
        $logs->endorse_id = $request->id;
        $logs->user_id = Auth::user()->id;
        $logs->position_id = Auth::user()->roles->first()->id;
        $logs->activity = 'EDIT THE ENDORSEMENTS DETAILS WITH '. $for_logs;
        $logs->remarks = '-';
        $logs->save();
        ...
        public function tfs_bi_get_name_user()
    {
        return response()->json(Auth::user()->name);
    }

    public function tfs_bi_client_fullci_submit_endorse(Request $request)
    {
        $removeScript = new ScriptTrimmer();
        $getAcctList = DB::table('bi_account_to_users')
            ->join('bi_account_list', 'bi_account_list.id', '=', 'bi_account_to_users.bi_account_id')
        ...
        $endorseId = DB::table('bi_endorsements')
            ...
            ([
                ...
                'contract_num' => $removeScript->scripttrim($request->contract_num),
                'bi_id' => $getAcctList[0]->bi_id,
                'type_of_endorsement_bank' => 'Full C.I',
                'bi_account_name' => $sitename,
                'project' => $sitename,
                'lob' => 'N/A',
                'package' => '-',
        ...
        $logs->user_id = Auth::user()->id;
        $logs->position_id = $user->roles->first()->id;
        $logs->activity = 'ENDORSED THE ACCOUNT(Full C.I)';
        $logs->remarks = '-';
        $logs->save();
    ...
    public function tfs_bi_check_double_endorse(Request $request)
    {
        $removeScript = new ScriptTrimmer();

        $biAccount = DB::table('bi_account_to_users')
            ->where('users_id', '=', Auth::user()->id)
            ->where('to_display', '=', 'display')
            ->select('bi_account_id')
            ->get()[0]->bi_account_id;


        $checkAccntName = DB::table('bi_endorsements')
            ->where('bi_id', '=', $biAccount)
            ->where('contract_num', '=', $request->contract_num)
            ->where('type_of_endorsement_bank', '=', $request->type_endo)
            ->count();

        return response()->json($checkAccntName > 0);
    }
}
