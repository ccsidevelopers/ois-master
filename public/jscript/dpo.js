$('#dpo_bi_rep_init').click(function ()
{
    var btn = $(this);
    btn.attr('disabled', true);
    $('#dpo_dispatchNa').attr('disabled', true);
    $('#modal_dpo_add_bi_note').modal('show');
    $.ajax({
        type: 'get',
        url: 'dpo-get-timedate',
        success: function(data)
        {
            $('#dpo_DateDue').val(data[0]);
            $('#dpo_timeDue').val(data[1]);
        },
        complete: function()
        {
            btn.attr('disabled', false);
            $('#dpo_dispatchNa').attr('disabled', false);
        }
    });
});
...
$('#dpo_dispatchNa').click(function ()
{
    $('#dpo_dispatchNa').prop('disabled', true);
    var checkingIfNull = false;
    var dataToPass = {}; //object data holder

    $('.dpo_inputs').each(function()
    {
       if($(this).val() != '')
       {
           checkingIfNull = true;
       ...
       {
           checkingIfNull = false;
           return false;
       }

       dataToPass[$(this).attr('what')] = $(this).val();
    ...
    if(checkingIfNull)
    {
        $.ajax({
            ...
            complete: function ()
            {
                alert('Succesfully Dispatch');

                $("#dpo_assignCI"). val(""). trigger("change");
                $('#dpo_ci_name').val('');
                $('#dpo_subj_name').val('');
                $('#dpo_DateDue').val('');
                $('#dpo_timeDue').val('');
                $('#modal_dpo_add_bi_note').modal('hide');
            }
        })
    }
    else
    {
        ...
        $('#dpo_dispatchNa').prop('disabled', false);
                ...
                "columns":
                    [
                        ...
                        {data: 'created_at', name: 'bi_ci_report.created_at'},
                        {
                            ...
                            {
                                   return '<button class="btn btn-sm btn-info btn-block view_bi_rep_logs"  href="'+data.id+'"><i class="glyphicon glyphicon-film"></i> Logs</button>';
                            },
