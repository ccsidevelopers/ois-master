...
{
    $('#incident-rep-table-pending thead th').each(function()
    {
        inc_rep_pen[inc_rep_pen_count] = $(this).text();
        inc_rep_pen_count++;
    });
    tableIncidentRepPending = $('#incident-rep-table-pending').DataTable
    ({
        ...
        "serverSide": true,
        "responsive": true,
        "ajax": "/gen_inc_rep_review_pending_approver",
        "columns":
            [
    ...
    $('#incident-rep-table-pending_filter input').unbind();
    $('#incident-rep-table-pending_filter input').bind('keyup change',function (e) {
...
{
    $('#incident-rep-table-reviewed thead th').each(function()
    {
        inc_rep_done[inc_rep_pen_count] = $(this).text();
        inc_rep_done_count++;
    });
    tableIncidentRepDone = $('#incident-rep-table-reviewed').DataTable
    ({
        ...
        "serverSide": true,
        "responsive": true,
        "ajax": "/gen_inc_rep_review_done_approver",
        "columns":
            [
                {data: 'id', name: 'incident_report.id'},
                {data: 'date_time', name: 'admin_requisition.date_request'},
        ...
        {
            if(aData.stat == 'Management Approved')
            {
                $(nRow).css('background-color', '#b3ffb3');
                // $(nRow).css('color', 'white');
            }
            else if(aData.stat == 'Management Reject')
            {
    ...
    $('#incident-rep-table-reviewed_filter input').unbind();
    $('#incident-rep-table-reviewed_filter input').bind('keyup change',function (e) {
...
$('#incident-rep-table-pending').on('click', '.btnViewInfoIncident', function()
{
    var id = atob($(this).attr('name'));
    var stst = $(this).attr('fl');

    $('#loopIncidentImages').html('');
    $('#incident_rep_rem_view').val('');
    $('#incident_rep_approver_remarks_view').val('')

    $('.btn1stApprovalIncident').attr('name', '');

    getImageRemarksIncident(id);

    if(stst == 'New')
    {
        $('#divHideShowIncidentButtons').show();
        $('.btn1stApprovalIncident').attr('name', $(this).attr('name'));
        $('#hideShowApproverRemarksView').hide();
    }
    else
    {
        $('#divHideShowIncidentButtons').hide()
        $('#hideShowApproverRemarksView').show();
    }

});

function getImageRemarksIncident(id)
{
    $.ajax
    ({
        type : 'get',
        ...
        success : function(data)
        {
            ...
            $('#incident_rep_rem_view').val(data[1][0].report_remarks);
            $('#incident_rep_approver_remarks_view').val(data[1][0].rem_approver);

            for(var i=0; i <= data[0].length-1; i++)
            {
                $('#loopIncidentImages').append
                (
...
$('.btn1stApprovalIncident').click(function()
{
    var btn = $(this);
...
$('#incident-rep-table-reviewed').on('click', '.btnViewInfoIncident', function()
{
    var id = atob($(this).attr('name'));
    var stst = $(this).attr('fl');
